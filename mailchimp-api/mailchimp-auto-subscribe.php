        ...
        if ( isset( $initial_fields['dt_mailchimp_subscribed_mc_lists']['values'] ) ) {
            ...
            foreach ( $subscribed_mc_lists as $list ) {
                ...
                try {
                    ...
                    dt_mailchimp_logging_add( 'Processing MC List ID: '. $mc_list_id . ' with status: ' . $status );
                    ...
                    if ( empty( $mc_records ) ) {
                        dt_mailchimp_logging_add( 'No corresponding MC Records Found!' );
                        ...
                        dt_mailchimp_logging_add( 'Found '. count( $mc_records ) .' corresponding MC Records!' );
                    }

                    $hidden_id_field_tag = Disciple_Tools_Mailchimp_API::get_default_list_hidden_id_field();
                    dt_mailchimp_logging_add( 'Hidden ID Field Tag: '. $hidden_id_field_tag );
                        ...
                        if ( ! empty( $mc_record ) ) {
                            ...
                            if ( isset( $mc_record->merge_fields->{$hidden_id_field_tag} ) && strval( $mc_record->merge_fields->{$hidden_id_field_tag} ) === strval( $post_id ) ) {
                                ...
                                if ( strtolower( $mc_record->status ) !== $status ) {
                                    ...
                                    dt_mailchimp_logging_add( 'No subscription mismatch detected!' );
                                }
                            } else {
                            ...
                            dt_mailchimp_logging_add( 'Empty MC Records Found!' );
                        }
                    }
